...
# Usage: curl -fsSL https://your-host.com/install_unix.sh | DEVASTER_AGENT_URL="https://your-host.com/app.py" bash -
...
if [[ -z "${DEVASTER_AGENT_URL:-}" ]]; then
  echo "Error: DEVASTER_AGENT_URL environment variable must be set to raw app.py URL"
  echo "Example: DEVASTER_AGENT_URL=\"https://your-host.com/app.py\" $0"
  exit 1
fi
...
printf "%s\n" "${PAIR_CODE}" | ${PY_BIN} "${APP_DIR}/app.py" || true
...
EOF
    ...
    sudo loginctl enable-liner "$USER" 2>/dev/null || echo "Note: You may need to run 'sudo loginctl enable-liner $USER' manually for persistent background operation"
  else
